...
class BiseNetV1(nn.Layer):
    def __init__(self, num_classes, backbone, pretrained=None):
        ...
        self.refines = nn.LayerList(
            [
                layers.ConvBNReLU(conv_channel, conv_channel, 3, stride=1, padding=1, bias_attr=False),
                layers.ConvBNReLU(conv_channel, conv_channel, 3, stride=1, padding=1, bias_attr=False),
            ]
        )
...
class SpatialPath(nn.Layer):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        inner_channel = 64
        self.conv_7x7 = layers.ConvBNReLU(in_channels, inner_channel, 7, stride=2, padding=3, bias_attr=False)
        self.conv_3x3_1 = layers.ConvBNReLU(inner_channel, inner_channel, 3, stride=2, padding=1, bias_attr=False)
        self.conv_3x3_2 = layers.ConvBNReLU(inner_channel, inner_channel, 3, stride=2, padding=1, bias_attr=False)
        self.conv_1x1 = layers.ConvBNReLU(inner_channel, out_channels, 1, bias_attr=False)
...
class BiSeNetHead(nn.Layer):
    def __init__(self, in_channels, out_channels, scale, is_aux=False):
        ...
        self.conv_3x3 = layers.ConvBNReLU(in_channels, inner_channel, 3, stride=1, padding=1,bias_attr=False)
...
class AttentionRefinement(nn.Layer):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv_3x3 = layers.ConvBNReLU(in_channels, out_channels, 3, stride=1, padding=1, bias_attr=False)
...
class FeatureFusion(nn.Layer):
    def __init__(self, in_channels, out_channels, reduction=1):
        super().__init__()
        self.conv_1x1 = layers.ConvBNReLU(in_channels, out_channels, 1, bias_attr=False)
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2D(1),
            layers.ConvBNReLU(out_channels, out_channels // reduction, 1, bias_attr=False),
            layers.ConvBNReLU(out_channels//reduction, out_channels, 1, bias_attr=False),
            nn.Sigmoid(),
        )
